...
RC408.registerModule({
  ...
  theory: `
> **真题考情**：页面置换算法直接推演考过 **7/17 年**（09/10/12 大题 + 15/19/21/25 选择），FIFO/CLOCK/LRU 三种都考过，2014 年还专考 **Belady 异常**；虚拟存储整体考频见「页式虚拟内存」模块（13/17 年）。

## 背景：请求分页存储管理
进程只用少量物理块（页框）运行，访问的页不在内存时产生**缺页中断**，由操作系统调入；
若物理块已满，必须按某种**置换算法**淘汰一页。缺页率是衡量算法优劣的核心指标。

## LRU 的思想
**淘汰"最长时间未被访问"的页面**（最近最久未使用）——依据程序的局部性原理：
最近很久不用的页，将来被访问的概率也小。

## 手算方法（考试通用）
- 方法一（**时间戳**）：给每页记录"最后一次被访问的时刻"，淘汰时刻最早者；
- 方法二（**栈 / 访次序列**）：维护一个页号序列，每次访问把该页移到序列**末尾**（最近使用端），
  序列**最前端**即最久未使用页 —— 本可视化采用此法，右侧"使用顺序"条就是它。

## 与 FIFO / OPT 对比
| 算法 | 淘汰规则 | 特点 |
| --- | --- | --- |
| FIFO | 最先进入的页 | 实现简单，可能出现 **Belady 异常**（块数增加缺页反而增多） |
| LRU | 最近最久未使用 | 堆栈类算法，无 Belady 异常；硬件开销较大 |
| OPT | 以后最长时间不被访问 | 理论最优，无法实现，用作评价基准 |

## 考点提示
- 命中的页也要**更新"最近使用"标记**——这是 LRU 与 FIFO 最容易混淆的地方；
- 缺页率 = 缺页次数 ÷ 访问总次数；
- 判断"某序列在 m 个物理块下是否缺页 n 次"是高频计算题，建议用表格逐列推。
`,
