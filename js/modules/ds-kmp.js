...
RC408.registerModule({
  ...
  theory: `
> **真题考情**：2015 年（失配后 i、j 的值）、2019 年（匹配总比较次数）、2024 年（"aabaab" 的修正 next）。
> 三题都围绕一个核心：**fail/next 数组的含义与手算**。

## fail / next 的两种口径（都给出，避免踩坑）
- **fail[j]**（0 基，本模块演示用）：模式串 p[0..j] 的**最长相等前后缀长度**；失配时 j 跳到 fail[j-1] 继续比较，**主串指针 i 不回退**；
- **教材 next[j]**（1 基）：next[1] = 0，next[j+1] = fail[j] + 1 —— 与 fail 只差一个平移；
- **nextval（修正值，2024 真题）**：若 p[j] == p[ next[j] ]，则 nextval[j] = nextval[ next[j] ]（避免无谓比较）。

## KMP 的优势
主串指针 i **从不回退**，比较次数 O(n + m)（暴力最坏 O(n×m)）——2019 年真题要求算出精确比较总次数。

## 本模块的观察要点
- 求 fail 时观察"最长相等前后缀"如何滑动扩展；
- 匹配时失配后模式串整体"滑动"，主串 i 始终不回退。
`,
