...
RC408.registerModule({
  ...
  theory: `
> **真题考情**：18 年中 8 年考寻址方式（11/13/14/15/16/18/20/23），固定题型：变址/基址求操作数、间接寻址访存次数、偏移寻址辨析（2011），并常与页式虚存、TLB、Cache 综合——"一条指令访存几次"（2015/2016 真题）是经典陷阱题。

## 寻址方式解决的问题
指令里给出的"形式地址 A"往往不是操作数的真实位置 —— **由 A 变换出有效地址 EA 的规则**就是寻址方式。
一条指令：\`OP | A\`，寻址方式由操作码隐含或由寻址特征位指出。

## 八种常考寻址方式
| 寻址方式 | 有效地址 / 操作数 | 取操作数访存 | 特点 |
| --- | --- | --- | --- |
| 立即寻址 | 操作数 = A 本身（#data） | 0 | 最快；A 位数限制范围 |
| 直接寻址 | EA = A | 1 | 简单；范围受 A 位数限制 |
| 间接寻址 | EA = (A) | 2 | 扩大范围；慢一次访存 |
| 寄存器寻址 | 操作数 = (R) | 0 | 最快；寄存器有限 |
| 寄存器间接 | EA = (R) | 1 | 指针的硬件原型 |
| 相对寻址 | EA = (PC) + A | 1 | 程序浮动；转移指令 |
| 基址寻址 | EA = (BR) + A | 1 | BR 由**操作系统**管理；重定位 |
| 变址寻址 | EA = (IX) + A | 1 | IX 由**用户**管理；适合数组循环 |

## 基址 vs 变址（高频辨析）
- **基址寻址**：BR 不变（系统设定），A 变 —— 面向**操作系统**，解决程序重定位；
- **变址寻址**：IX 变（用户修改），A 不变 —— 面向**用户**，解决数组循环访问。

## 考点提示
- 相对寻址中 PC 的值：转移指令取指后 PC 已**自动 +1**（指向下一条），教材公式里的 (PC) 是"取指后"的值；
- "一次间接寻址"取操作数共访存 2 次（取 EA 一次 + 取数一次），不含取指令那次；
- 根据指令的执行结果反推寻址方式，是大题常见形式。
`,
