...
RC408.registerModule({
  ...
  theory: `
> **为什么要有它**：要在连通带权图里选出总权值最小的 n−1 条边（最省成本的布线 / 建网）——Kruskal 按"边"从小到大贪心，边少时最快。
...
| 项目 | 结果 | 说明 |
| --- | --- | --- |
| 时间 | \\(O(E\\log_2 E)\\) | 排序主导；并查集（路径压缩）近似 \\(O(\\alpha)\\) |
| 空间 | \\(O(V+E)\\) | 边集数组 + 并查集数组 |

- **Kruskal**：按"边"贪心，适合**稀疏图**；**Prim**：按"顶点"扩张（每次选离树最近的顶点），适合**稠密图**；
- 两者都要求图连通；不连通时得到**最小生成森林**。
...
4. Prim 适合稠密图、Kruskal 适合稀疏图，选择题常换个说法考同一句话。

> **真题考情**：**5/18 年（选 3 题 + 大 2 道）**：选 2012-8、2015-6、2020-7；大 2017-42（Prim 的贪心
> 正确性与算法设计）、2018-42（最经济布线方案，最小生成树有两种形态，画图并算总权值）。
`,
