...
RC408.registerModule({
  ...
  theory: `
> **真题考情**：18 年中 **13 年**考 Cache，**大题几乎每年一道**（10/11/16/18/20/23/25 年均考综合大题）；直接映射出现在 2010/2016/2021，近年大题更偏组相联 + 虚拟存储综合（规划中的"组相联"与"页式虚拟存储"模块衔接）。常考计算：地址划分位数、总容量、命中率、缺失次数。

## 为什么需要 Cache
CPU 与主存存在速度鸿沟，Cache（高速缓存）依据**局部性原理**（时间局部性 + 空间局部性）
把正在使用的主存块副本放在离 CPU 更近的小容量快速存储中。

## 直接映射规则
主存块只能装入**唯一确定的** Cache 行：
\`\`\`text
...
| 字段 | 位数 | 作用 |
| --- | --- | --- |
| 块内偏移 Offset | log₂(块大小) | 在块内寻址具体字节 |
| 行号 Index | log₂(行数) | 直接指出该块映射到哪一行 |
| 标记 Tag | 其余高位 | 判断该行缓存的**是否就是**要找的块 |

## 访问判命中
1. 由地址取出 Index，定位 Cache 行；
2. 该行**有效位 = 1** 且 **Tag 与地址 Tag 相同** → **命中**，直接读 Cache；
3. 否则**缺失**：把主存块调入该行（无有效位 → 强制/冷缺失；有效位=1 但 Tag 不同 → **冲突缺失**，旧块被无条件替换）。

## 直接映射的优缺点
- 优点：硬件极简单、无需替换算法、访问速度快；
- 缺点：不够灵活——两个频繁交替访问的块若映射到**同一行**，会反复互相踢出（**抖动 / Thrashing**），
  这也是全相联、组相联映射要解决的问题。

## 考点提示
- 已知地址位数、行数、块大小，**划分三段位数**并计算 Tag/Index/Offset 的值是必考题；
- 命中率 = 命中次数 ÷ 访问总次数；注意区分强制缺失与冲突缺失。
`,
