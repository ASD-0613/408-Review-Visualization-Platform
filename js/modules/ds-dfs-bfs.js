...
RC408.registerModule({
  ...
  theory: `
> **为什么要有它**：图没有"第一个元素"，要系统地访问每个顶点一次、并得到生成树 / 森林，就得先规定搜索次序——DFS 与 BFS 是最基本的两种。
> **怎么实现**：都用 visited 数组 + 一个容器——DFS 用**栈**（沿第一条没走过的边深入，走不通就回溯），BFS 用**队列**（访问邻点后逐层向外扩散）。
> **记住什么**：DFS 栈 / BFS 队列别记反 + 复杂度（邻接表 \\(O(n+e)\\)、邻接矩阵 \\(O(n^2)\\)）+ **BFS 层号 = 无权图最短路径**。

## 统一框架
- 外层：仍有未访问顶点时，从编号 / 名字最小的那个重新开始 → 非连通图得到**生成森林**，**搜索轮数 = 连通分量数**；
- **DFS**：访问 v 后沿"第一个未访问邻点"深入；v 的邻点全访问过 → 退栈**回溯**（递归实现的系统栈就是它）；
- **BFS**：访问 v 后把 v 的所有未访问邻点依次访问并**入队**，再取队首重复 → **逐层扩散**。

## 复杂度（邻接表存储是常考点）
| 存储结构 | 时间 | 原因 |
| --- | --- | --- |
| 邻接表 | \\(O(n+e)\\) | 每个顶点进容器一次，每条边（无向图两个方向）各检查一次 |
| 邻接矩阵 | \\(O(n^2)\\) | 每个顶点都要扫一整行才能找出全部邻点 |

空间都是 \\(O(n)\\)（visited 数组 + 容器）。

## 序列的唯一性
- **同一个图，DFS / BFS 序列都不唯一**——取决于起点，也取决于邻接表中邻接点的排列次序；
- 采用**邻接矩阵**（按下标从小到大扫）时序列唯一；本模块的邻接表按顶点名升序，等价于升序扫描；
- 判"给定序列是否合法"：逐步模拟，看是否满足"沿边深入 / 逐层扩散"。

## 生成树 · BFS 与无权最短路
- 遍历真正走过的 n−1 条边构成**生成树**（非连通图则是生成森林）；
- **BFS 生成树是以起点为根的层序树，树高最小；DFS 生成树一般较高**；
- BFS 的层号（距起点的边数）**就是无权图上从起点到该顶点的最短路径长度**——求无权单源最短路只能用 BFS；
- 对 DAG 做 DFS，**退栈序列恰好是逆拓扑有序序列**。

## 考点提醒（易错点）
1. 一定问清"**从哪个顶点出发**"——换起点序列就变；
2. **DFS 用栈、BFS 用队列**，别记反；栈里放的是当前搜索路径，队列里放的是待扩展的一层；
3. 非连通图别漏后面的分量：生成森林边数 = \\(n\\) − 分量数；
4. 判"某序列能否是 BFS 序列"：按层检查——**同层顶点之间不能有边**（否则会被提前访问）。

> **真题考情**：**6/18 年（全为选择题）**：2012-5（邻接表下 BFS 的复杂度）、2013-8（序列判定）、
> 2015-5（有向图 DFS 序列种数）、2016-6（序列合法性）、2020-6（退栈序 = 逆拓扑序）、2023-6（边权为 1 的最短路）。
`,
