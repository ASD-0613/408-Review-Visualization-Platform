...
RC408.registerModule({
  ...
  theory: `
> **真题考情**：18 年中 **8 年**考到（选择题 **7** 道 + 大题 **1** 道）——2009-9（小根堆插入 3 后调整）、2011-11（大根堆尾部插入 18 的**比较次数**）、2015-10（小根堆删除堆顶后重建的**比较次数**）、2018-11（序列建成大根堆的**序列变化过程**）、2020-9（大根堆叙述辨析：完全二叉树 / 顺序存储 / 次大值位置）、2021-11（依次插入初始为空的大根堆）、2024-9（大根堆**两次删除**后的新堆）、大题 **2022-42**（找最小的 10 个数——题干没写"堆"，但标准答案之一就是**用含 10 个元素的大根堆**维护当前最小的 10 个）。**插入 / 删除堆顶各只走一条路径，正是那三道"比较次数"题的共同套路。**
> ⚠ 按"选择题尽量只归一个知识点"的口径，2018-11 与 2024-9 从「排序算法全家桶」移到了本模块（它们问的是堆的结构调整，不是排序算法性质）；而**只问排序性质、堆排序仅作选项**的题（2012-10、2016-11、2017-11、2023-10）仍归排序模块。18 年真题**从未直接考"优先队列"这个词**。

## 堆的定义（大根堆 / 小根堆）
- **堆是一棵完全二叉树**，且每个结点都不小于（或不大于）它的两个孩子：
  - **大根堆**：\\(a_i \\ge a_{2i}\\) 且 \\(a_i \\ge a_{2i+1}\\)；**小根堆**：\\(a_i \\le a_{2i}\\) 且 \\(a_i \\le a_{2i+1}\\)；
- 只需检查 **\\(i = 1 \\sim \\lfloor n/2 \\rfloor\\)** 这些**非叶结点**（叶结点没有孩子，天然满足）；
- 堆**只约束"双亲 vs 孩子"**，**不保证**左孩子 ≥ 右孩子，**也不保证**同一层有序 —— 这是最常见的陷阱；
- 堆**允许重复关键字**（对比：二叉排序树要求互不相同）。

## 顺序存储（完全二叉树才能这么存）
**下标从 1 开始**（408 / 严蔚敏教材口径）：

| 关系 | 公式 |
| --- | --- |
| 双亲 | \\(\\lfloor i/2 \\rfloor\\)（\\(i>1\\)） |
| 左孩子 | \\(2i\\)（\\(2i \\le n\\) 才存在） |
| 右孩子 | \\(2i+1\\)（\\(2i+1 \\le n\\) 才存在） |
| 是否为叶 | \\(2i > n\\) |
| 最后一个非叶结点 | \\(\\lfloor n/2 \\rfloor\\) |

- n 个结点的堆**树高 = \\(\\lfloor \\log_2 n \\rfloor + 1\\)**；
- **堆的"大小 n"与"数组容量"是两回事**：删除堆顶只减小 n，数组空间不动；
- 若题目用 **0 基下标**：孩子为 \\(2i+1, 2i+2\\)，双亲为 \\(\\lfloor (i-1)/2 \\rfloor\\)（先看清题目的下标起点）。

## 三个基本操作
**① 建堆（自底向上筛选 / Floyd 算法）**
- 从**最后一个非叶结点 \\(\\lfloor n/2 \\rfloor\\)** 开始，**倒着**到根，逐个"向下筛选"；
- 依据：做 \\(i\\) 时它的左右子树**已经各自是堆**，所以只需把 \\(a_i\\) 一路下沉到合适位置；
- 时间 **\\(O(n)\\)**，**不是 \\(O(n\\log_2 n)\\)**：高度 h 的结点至多 \\(\\lceil n/2^{h+1} \\rceil\\) 个，
  \\(\\sum_h h \\cdot n/2^{h+1}\\) 收敛到 O(n)。

**② 插入（放末尾 → 向上调整）**
- 新元素放到 \\(a_{n+1}\\)（完全二叉树的"下一个空位"），然后与双亲比较，比双亲更优就交换、继续往上；
- 最多走一条根到叶的路径：**\\(O(\\log_2 n)\\)**。

**③ 删除堆顶（取极值）**
- 堆顶就是极值，取走是 **\\(O(1)\\)**；
- 把**末尾元素**搬到堆顶、n 减 1，再把堆顶**向下筛选**：**\\(O(\\log_2 n)\\)**。

**建造方式对比（常考）**：把 n 个元素**逐个插入**空堆是 \\(O(n\\log_2 n)\\)；**Floyd 自底向上建堆是 \\(O(n)\\)** —— 后者更快。

## 优先队列
- 优先队列 = "每次取出的都是优先级最高（或最低）的元素"的队列，**堆是它最常用的实现**：
  插入 \\(O(\\log_2 n)\\)、取极值 \\(O(1)\\)、删除极值 \\(O(\\log_2 n)\\)；用有序数组则插入 \\(O(n)\\)、用无序数组则取极值 \\(O(n)\\)；
- 应用：堆排序、Top-K、任务调度、Dijkstra / Prim 的"取最小"、哈夫曼树的"取两个最小"。

## 考点提示
- 判断"某序列是否为堆"：**从 \\(i=1\\) 查到 \\(\\lfloor n/2 \\rfloor\\)**，逐个比双亲与两个孩子，别漏掉右孩子、别多查叶结点；
- 问"建堆后的结果"：一定要**从 \\(\\lfloor n/2 \\rfloor\\) 倒着做**，中间过程会影响最终形态（自根向下做是错的）；
- 问"插入/删除后堆的变化"：插入只看**一条向上路径**，删除只看**一条向下路径**，与路径外的结点无关；
- 与**堆排序**区分：本模块演示"堆的结构与调整"，堆排序（每趟堆顶与末尾交换、尾部有序区扩大）见 **排序算法全家桶**。
`,
