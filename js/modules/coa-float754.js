 ...
 * ----------------------------------------------------------------------------
 * 真题考情：2009-2025 中 11 年考 IEEE 754（09/11/12/13/14/16/18/20/21/23/25），
 * 固定题型两种："十六进制机器数 → 求真值" 与 "十进制 → 机器数"，
 * 且 2023 年考到**非规格化数**、2019 年考到**精度极限**——本模块均覆盖。
 *
 * 快照设计（decode 求值方向）：
...
RC408.registerModule({
  ...
  theory: `
> **真题考情**：18 年中 **11 年**考 IEEE 754，固定两问：十六进制机器数 ↔ 真值
> 互求；2023 年考**非规格化数**（8020 0000H），2019 年考**精度**（1.2 无法精确表示）。

## 格式：符号 + 阶码 + 尾数
| 类型 | 总位数 | 符号 S | 阶码 E（含偏置） | 尾数 M | 偏置值 |
| --- | --- | --- | --- | --- | --- |
| 单精度 float | 32 | 1 | 8 | 23 | 127 |
| 双精度 double | 64 | 1 | 11 | 52 | 1023 |

$$N = (-1)^S × 1.M × 2^{E-偏置}$$（**规格化数**隐含最高位"1."，不占存储——多赚一位精度）

## 四类特殊值（由 E 与 M 联合判定）
| E（阶码字段） | M | 含义 |
| --- | --- | --- |
| 全 0 | 全 0 | ±0 |
| 全 0 | 非 0 | **非规格化数**：N = (-1)^S × 0.M × 2^(1-偏置)（2019 真题） |
| 全 1 | 全 0 | ±∞（2019 年 f2(127) 考点） |
| 全 1 | 非 0 | NaN |
| 1 ~ 254 | 任意 | 规格化数（正常公式） |

## 手算套路（decode：机器数 → 真值）
1. 展开二进制，切 1/8/23 三段（以单精度为例）；
2. E 字段按**无符号**读出，减 127 得真实指数；
3. 尾数字段前补 "1." 得 1.M；
4. 代入公式。**注意符号位在最前面**（2023 年真题 -0.4375、2025 年 4730 0000H 均为常规题）。

## 本模块的观察要点
- 三段颜色：🔴 符号 / 🟡 阶码 / 🟢 尾数，与 408 大题的书写方式一致；
- 非规格化数的"0.M"与隐含位消失、阶码固定 1−偏置，是失分重灾区。
`,
