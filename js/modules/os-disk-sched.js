...
RC408.registerModule({
  ...
  theory: `
> **为什么要有它**：磁头是机械部件，请求的服务顺序直接决定寻道总距离（占总存取时间的最大头）。
> **怎么实现**：按规则重排请求序列——FCFS / SSTF / SCAN / C-SCAN（本模块四种）。
> **记住什么**：**平均寻道长度 = 移动总磁道数 ÷ 请求数**——整道题的核心计算只有这一个。
...
## 考点提醒（易错点）
1. 手算先画 **0~199 的磁道轴**，把请求标上去再按规则走一遍（本模块的可视化就是这张图）；
2. **折返/返回帧算不算移动**要看题目口径：本演示按王道口径**计入总量**，个别教材不计返回过程；
3. **LOOK / C-LOOK** 到"最远的请求"就折返（不必扫到边界）——题目说 SCAN 时要看清是严格版还是 LOOK；
4. 平均寻道长度的分母是 **请求数**（不是磁道数）；SSTF 的"最近"用 |目标 − 当前| 比较，同距任选。

> **真题考情**：**6/18 年，大题 2 道**（2010-45、2019-44；选 2009-29 SCAN、2015-32 SCAN、
> 2021-26 SSTF、2024-32 C-SCAN），四种算法都考过、且大题小题都是"算总磁道数"。
`,
