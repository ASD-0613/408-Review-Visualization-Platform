 ...
 * 考情：2012-7（确定次序）、2016-8（依次确定）、2014-42/43 大题（OSPF→最短路径计算）。
...
RC408.registerModule({
  ...
  theory: `
> **真题考情**：2012 年（确定目标顶点的次序）、2016 年（依次确定的顶点）选择题；
> 2014 年 42/43 大题以 OSPF 链路状态为背景考最短路径计算与路由表。
> 2013 年真题辨析：**边权全为 1 时可用 BFS** 代替。

## 算法（贪心，要求权值非负）
\`\`\`text
dist[源]=0，其余 ∞；集合 S = ∅
每轮：① 取未确定顶点中 dist 最小的 u → 并入 S（其 dist 即最短距离）
      ② 松弛 u 的每条出边 (u,v,w)：若 dist[u]+w < dist[v]，更新 dist[v]=dist[u]+w，prev[v]=u
重复 n 次。时间 O(n²)（邻接矩阵）或 O((n+e)log n)（堆优化）。
\`\`\`

## 考点提示
- 每轮"确定"一个顶点，**确定次序由 dist 最小决定**，不由编号决定（2012 真题）；
- 已确定顶点的 dist 不再变化（贪心正确性依赖**非负权**；有负权边必须改用 Floyd/BF）；
- 求具体路径：由 prev 前驱数组从终点回溯（2016 真题）。
`,
