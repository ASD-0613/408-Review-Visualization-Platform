 ...
 * 考情：18 年中 12 年考（2010-4/6、2013-4、2015-3、2017-6、2018-5、2021-5、
 * 2022-5、2023-4、2025-5 选择，2012-41 大题考贪心合并思想）。
...
RC408.registerModule({
  ...
  theory: `
> **真题考情**：18 年中 **12 年**考哈夫曼（2010 两题、2012 大题、2013/2014/2015/2017/2018/2019/2021/2022/2023/2025 各 1 题），
> 固定题型：**求 WPL、求编码长度、判断编码序列译码结果**。

## 构造算法（贪心）
n 个权值看作 n 棵单结点树：每步**取出权值最小的两棵**合并（新根权值 = 两者之和），
放回集合重复，直到只剩一棵。n 个叶子的哈夫曼树共 **2n−1 个结点**，**没有度为 1 的结点**。

## WPL 与编码
- **WPL**（带权路径长度）= Σ 权值 × 到根的路径长度，哈夫曼树是 WPL 最小的二叉树；
- 编码：左分支记 **0**、右分支记 **1**（约定可互换，长度不变）；频率越大的字符编码越短；
- **前缀码**：任何编码都不是另一编码的前缀 → 译码无歧义（哈夫曼编码必为前缀码）。

## 高频结论（2010/2012/2022 真题）
- 权值集合相同、合并顺序不同 → 树形可能不同，但 **WPL 唯一**；
- 三叉哈夫曼树需要补**权值 0 的虚结点**：补 (k−1)−((n−1) mod (k−1)) 个（2013 真题）；
- 编码序列译码：从根出发按 0/1 走到叶子即还原一个字符，回到根继续（2017 真题）。
`,
