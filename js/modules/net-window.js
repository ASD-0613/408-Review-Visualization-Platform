...
RC408.registerModule({
  ...
  theory: `
> **真题考情**：18 年中 **10 年**考（9 道选择 + 1 道大题）——
> 选择 2009-35、2011-35、2012-36、2014-36、2015-35、2018-36、2019-35、2020-36、2024-37；
> 大题 2017-47（双向 GBN + 捎带确认，求最大信道利用率）。

## 一、三种协议的窗口（W_T 发送窗口 / W_R 接收窗口）
| 协议 | 发送窗口 W_T | 接收窗口 W_R | 收到失序帧怎么办 | 确认方式 |
| --- | --- | --- | --- | --- |
| 停止-等待 | 1 | 1 | — | 每帧一个确认 |
| **GBN 后退 N 帧** | ≤ 2ⁿ − 1 | **1** | **丢弃**，并重复 ACK 最后一个按序帧 | 累积确认（ACK n = n 及之前全部收到） |
| **SR 选择重传** | ≤ 2ⁿ⁻¹ | ≤ 2ⁿ⁻¹（一般取 W_R = W_T） | **缓存**，等缺口补齐后一并上交 | 逐帧独立确认（ACK n 只代表 n 收到了） |

## 二、序号位数 n 与窗口上限（必考）
- **GBN**：W_T ≤ 2ⁿ − 1（等价于 序号个数 ≥ 发送窗口 + 1）。若 W_T = 2ⁿ，发送窗口与接收窗口完全重叠，接收方分不清收到的是新帧还是重传帧；
- **SR**：W_T = W_R ≤ 2ⁿ⁻¹，等价于 **W_T + W_R ≤ 2ⁿ**；
- 反向推导要看题目问什么：**求"至少几位序号"用 2ⁿ ≥ W + 1（GBN）或 2ⁿ⁻¹ ≥ W（SR）**。

## 三、信道利用率
设帧长 L（bit）、数据速率 R、单向传播时延 D、确认帧发送时延 T_a：
\\[T_d=\\frac{L}{R},\\qquad RTT=2D,\\qquad U=\\min\\left(1,\\ \\frac{W_T\\cdot T_d}{T_d+RTT+T_a}\\right)\\]
- **停止-等待**即上式取 W_T = 1；**连续 ARQ（GBN/SR）** 用窗口去填满"发送 + 往返 + 确认"这个周期；
- 忽略确认帧开销时 T_a = 0；**捎带确认**（双方都发数据，2017-47）时 T_a = T_d，周期 = 2T_d + 2D；
- 经典反推：要求 U ≥ 80% → W_T ≥ ⌈0.8 × (T_d + RTT + T_a) / T_d⌉，再按第二节定序号位数。

## 四、真题锚点表（做题前先对表，10 年的考法都在这里）
| 年份-题号 | 考什么 | 结论 |
| --- | --- | --- |
| 2009-35 | GBN 累积确认 | 收到 ACK3 就表示 0~3 全部收到；**ACK1 丢失不影响**（累积确认覆盖），要重传的是 4~7 |
| 2011-35 | SR 超时重传帧数 | 只收到 1 号确认、0 与 2 号超时 ⇒ 重传 **2 帧**（0、2）；3 号计时器未超时，不重传 |
| 2012-36 | 利用率最高 ⇒ 序号位数 | T_d = 64ms、RTT = 540ms、T_a = 64ms ⇒ 一个周期能发 668/64 ≈ 10.4 帧 ⇒ 2ⁿ ≥ 11.4 ⇒ **n = 4** |
| 2014-36 | GBN 最大平均速率 | W = 1000 帧 × 1000B = 1MB，一个 RTT（100ms）发完 ⇒ 80Mbps（再与带宽取小） |
| 2015-35 | 利用率 ≥ 80% ⇒ 序号位数 | n×62.5 / (62.5 + 500) ≥ 80% ⇒ n ≥ 7.2 取 8 ⇒ 2ᴷ ≥ n+1 = 9 ⇒ **K = 4** |
| 2017-47 | 双向 GBN + 捎带确认（大题） | 3 bit 序号 ⇒ 窗口 ≤ 7；U = N·T_d / (T_d + RTT + T_a) = 7×0.08 / (0.08+0.96+0.08) = **50%** |
...
| 2024-37 | SR 窗口滑动条件 | 3 bit ⇒ W = 4；缺 ACK1 时窗口不滑动，F1 超时后重传 F1（答案 D：F4、F1） |

## 五、考点提醒
- **GBN 的代价在重传、SR 的代价在缓存**：GBN 失一帧就要把窗口内其后所有帧全部重发；SR 只重发那一帧；
- **ACK 丢失不一定有害**：GBN 靠累积确认，后面任何一个 ACK 都能覆盖它（2009-35）；SR 的独立确认则必须由该帧自己的 ACK 到达，否则只能等超时（2024-37）；
- **GBN 的重复 ACK 不做快速重传**：接收窗口 = 1，重复 ACK 只表示"我还在等某一帧"，重传只能等超时；
- 帧序号是 **mod 2ⁿ 循环使用**的：本演示"序号空间"一行会显示窗口跨过序号末尾时的回绕。
`,
