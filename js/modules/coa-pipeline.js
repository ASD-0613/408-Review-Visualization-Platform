 ...
 * 考情：17 年中 8 年考（2012-44 大题、2025-19、2023-19、2019-18/19、2016-19、
 * 2014-44、2010-19），固定考点：数据冒险判定（RAW）、转发/气泡、时空图周期数计算。
 *
...
RC408.registerModule({
  ...
  theory: `
> **真题考情**：17 年中 8 年考（2012-44 大题画流水线图、2023-19 判断哪些指令阻塞、
> 2025-19 数据冒险叙述、2019-18/2016-19 冒险对判定），核心三问：
> **哪些指令对存在数据冒险 / 需要 n 个气泡 / 总共多少时钟周期**。

## 五段流水线
\`\`\`text
IF 取指 → ID 译码/取数 → EX 执行 → MEM 访存 → WB 写回
理想情况：每条指令 5 段各占 1 拍，满流后每拍完成 1 条
n 条无冒险指令总周期 = 5 + (n − 1)；加速比 ≈ 5
\`\`\`

## 数据冒险（RAW：写后读）——408 只考这一种判定
第 i 条指令**写** Rₓ，第 i+k 条（k≥1）**读** Rₓ → 冒险。
- **转发（Forwarding）**：EX/MEM 或 MEM/WB 段的结果直接前递到下一条的 EX 输入，
  大多数 RAW 无需停顿（2012 真题"无转发"条件）；
- **load-use 冒险**：load 的数据 MEM 段末才有，紧随其后的使用指令**即使转发也必须停 1 拍**；
- 无转发时：每个 RAW 冒险需**插入气泡**，直到写回完成（保守：2 个气泡）。

## 结构冒险 / 控制冒险（概念辨析）
- 结构冒险：硬件资源冲突（如指令/数据存储器合一）→ 分离 I-Cache/D-Cache 解决；
