 ...
 * 考情：18 年中 13 年考（2011-41 大题关键路径、2024-41 大题拓扑唯一性、
 * 2013-9/2019-5/2022-7 等选择题考 AOE 时间参数）。拓扑排序用 Kahn 入度消减；
...
RC408.registerModule({
  ...
  theory: `
> **真题考情**：18 年中 **13 年**考（2011-41 大题求关键路径及长度、2024-41 大题判断拓扑序列
> 是否唯一；2010/2013/2016/2018/2019/2020/2021/2022/2025 选择考拓扑性质或 AOE 时间参数）。

## 拓扑排序（Kahn 算法）
\`\`\`text
① 选一个入度为 0 的顶点输出；② 删除它及其所有出边（后继入度 −1）；重复 ①②。
\`\`\`
- 每步"可选顶点"可能不唯一 → **拓扑序列不唯一**；若中途无可输出顶点 → 图**有回路**（不是 DAG）；
- 时间 O(n + e)（邻接表）。DFS 的**退出序逆序**也是拓扑序列（2020 真题）。

## 关键路径（AOE 网）
边 = 活动（工期 w），点 = 事件。**工程最短工期 = 汇点的 ve**。
- **ve(j)** = 事件最早发生时刻 = \(\max\{v_e(i) + w_{ij}\}\)（按拓扑序正向推）；
- **vl(i)** = 事件最迟发生时刻 = \(\min\{v_l(j) - w_{ij}\}\)（按逆拓扑序反向推）；
- 活动 <i,j>：最早开始 e = ve(i)，最迟开始 l = vl(j) − w；**l − e = 0 的活动是关键活动**，
  全部关键活动构成**关键路径**——缩短非关键活动工期不能缩短总工期（2025 真题）。
`,
