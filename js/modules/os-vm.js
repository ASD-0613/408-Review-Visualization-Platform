...
RC408.registerModule({
  ...
  theory: `
> **为什么要有它**：程序比主存大也要能跑——只把**部分页**装入内存，其余留在磁盘，访问不到再按需调入。
> **怎么实现**：把逻辑地址切成"页号 p + 页内偏移 d"，先查快表 TLB、再查页表，缺页由中断调入。
> **记住什么**：**一次访问的完整链路**（下面那张图）+ **EAT 公式**——这是全卷分值最高的单一考点。
...
## 有效访问时间（EAT）
- 命中率 h、查 TLB 时间 t、访存一次时间 m：**EAT = h×(t+m) + (1−h)×(t + 2m)**
  （未命中要先查页表再访存；缺页还要加调页开销）；
- TLB 命中率常达 90% 以上——"页表在内存、TLB 在 CPU 内"，是"以小博中"的经典设计。

## 考点提醒（易错点）
1. 给逻辑地址求**页号 / 页内偏移 / 物理地址**：除以页大小取整 / 取余，或直接按二进制切分（按位数切最快）；
2. **TLB 命中 → 不查页表**；**TLB 未命中 → 仍要查页表**（快表是缓存，不是页表的替代）；
3. 缺页处理四步要能复述：**保护现场 → 从磁盘调页 → 更新页表 → 重新执行被中断的指令**；
4. 与计组交叉：缺页 + Cache 的访存次数（2016-45 那类综合题）按"TLB → 页表 → Cache → 主存"逐级数。

> **真题考情**：**14/18 年**（**大题 12 道**：2009-46、2010-46、2012-45、2013-46、2014-45、2015-46、
> 2016-45、2017-45、2018-44、2020-46、2024-45、2025-46；**选** 2022-15、2026-24·28·30），横跨计组与 OS 两科。
`,
