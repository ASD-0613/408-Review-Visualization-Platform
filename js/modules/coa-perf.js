 ...
 * 考情：18 年中 7 年直接考性能公式（全为选择：2012-12、2013-12、2017-12、2022-12、2023-12、
 * 2025-18、2026-20），变体包括"多类指令比例求平均 CPI"、"提速后再算总时间"。
 * （**窗12** 勘误：原写"6 年"且年份表漏 2026-20、把 2025 的题号写成 12。）
 * ========================================================================== */
...
RC408.registerModule({
  ...
  theory: `
> **为什么要有它**：同一条程序在不同机器上快不快，不能只看主频——**指令条数、CPI、主频三者共同决定执行时间**，性能指标就是把这三者的关系写清楚。
> **怎么实现**：CPU 执行时间 = 指令条数 × CPI ÷ 主频；多类指令先按**执行占比**加权求出平均 CPI。
> **记住什么**：三个公式之间的互相换算（执行时间 / MIPS / FLOPS）+ **MIPS 高不等于快**（不同指令集之间不能比）。

## 三个核心公式（必背）
- **CPU 执行时间 = 指令条数 × CPI ÷ 主频**；主频 = 1 ÷ 时钟周期（1 GHz 即每周期 1 ns）；
- **MIPS = 主频 ÷ (CPI × 10⁶)** = 指令条数 ÷ (执行时间 × 10⁶)；
- **FLOPS 系列**同理数"每秒浮点操作次数"（MF / GF / TF / PF，每级 ×1000）；
- **平均 CPI = Σ(CPIᵢ × 该类指令的占比ᵢ)**——占比按**执行条数**加权，不是按指令种类平均。

## 考点提醒（易错点）
1. **MIPS 高 ≠ 一定快**：不同指令集（不同 ISA）之间的 MIPS 没有可比性；
2. 提速题按比例直接算："指令数减 30%、CPI 变 1.2 倍，总时间是多少"；
3. 基准程序里**只有一部分**被提速时，要把两段分别算出来再相加；
4. 注意 **CPI / 时钟周期 / 主频**三者的辨析（2026-20 考 CPI 关系）。

> **真题考情**：**7/18 年，全为选择题**（2012-12、2013-12、2017-12、2022-12、2023-12、2025-18、2026-20），
