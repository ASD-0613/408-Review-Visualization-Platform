...
RC408.registerModule({
  ...
  theory: `
> **真题考情**：18 年中 **13 年**考到（选择题 13 道 + 大题 2 道）——2009-4（平衡条件 \\(|BF|\\le1\\)）、2010-4（RL 型需**两次**旋转）、2011-7（查找路径合法性）、2012-4（BF 全为 +1 时高 6 的最少结点数）、2013-3（构建 AVL 数平衡因子为 0 的分支结点）、2013-6（BST 删除后再插入）、2015-4、2018-6（中序递增）、2019-4（AVL 删除再插入，T₁ 与 T₃ 的关系）、2020-5（由插入序列反推 BST 形态）、2021-6（插入 23 后根中的关键字）、2024-7、2026-8（高 4 的 AVL 中根左右子树结点数最多相差几）；大题 **2022-41**（判定顺序存储的二叉树是否为二叉搜索树）与 **2026-41**（找与 K 之差的绝对值最小的所有结点）。

## 二叉排序树（BST / 二叉搜索树）
**定义**：左子树上所有关键字 < 根 < 右子树上所有关键字，且左右子树也分别是 BST。
- **中序遍历得到严格递增序列** —— 这是最好用的判据：2018-6 用它比关键字，**2022-41 直接用它判定"是否二叉搜索树"**（只需中序遍历一遍看是否递增，时间 \\(O(n)\\)、空间 \\(O(h)\\)）；
- 查找：从根开始比较，相等即命中，小于走左、大于走右；平均 \\(O(\\log_2 n)\\)；
- **插入的新结点一定是叶子**（一路比较到空指针才落位）；
- **删除**分三种：① 叶子直接删；② 只有一个孩子 → 用孩子顶替；③ 两个孩子 → 用**中序前驱（左子树最右）或中序后继（右子树最左）**顶替，再删掉那个前驱/后继。
  删除后再把同一个关键字插回来，**不保证还原成原来的树**（2013-6 就考这一点）；
- **最坏情况**：插入序列本身有序时 BST 退化成单支链，树高 \\(n\\)，查找退化成 \\(O(n)\\) —— 这正是 AVL 要解决的问题。

## 平衡二叉树（AVL）
- **平衡因子** \\(BF = h_L - h_R\\)（左子树高 − 右子树高）；AVL 要求**每个结点** \\(|BF| \\le 1\\)（2009-4）；
- 插入后从新结点**向上**找到第一个 \\(|BF| = 2\\) 的结点 z，它就是**最小不平衡子树的根**；再看 z 的较高孩子 y 的 BF 判型：
...
- 右旋：y（z 的左孩子）上升为子树根，z 下降为 y 的**右**孩子，y 原来的右子树改挂为 z 的**左**子树；
  左旋左右对称。**LR/RL 要两次**（2010-4 的原话："需要做两次旋转操作"）；
- **旋转不改变中序序列**（所以左旋/右旋只是把子树"压扁"，关键字相对次序不变）；
- **插入最多调整一次**（一次单旋或一次双旋）就能让整棵树恢复平衡；删除可能要多轮。

## 高度与最少结点数（2012-4 / 2026-8）
设 \\(N(h)\\) 为**高度 h 的 AVL 树的最少结点数**：\\(N(1)=1,\\ N(2)=2,\\ N(h)=N(h-1)+N(h-2)+1\\)。

| h | 1 | 2 | 3 | 4 | 5 | 6 | 7 |
| --- | --- | --- | --- | --- | --- | --- | --- |
| N(h) | 1 | 2 | 4 | 7 | 12 | 20 | 33 |

- **2012-4**：高度 6 且所有非叶结点 BF 全为 +1（即每个结点都取最少结点的那种形状）→ 结点总数 \\(N(6)=20\\)；
- **2026-8**：高度 4 的 AVL 中，**根的左右子树结点数相差最多**是多少？
  树高 4 ⟹ 较高那棵子树高 3（最多 \\(2^3-1=7\\) 个结点）；另一棵因 \\(|BF|\\le1\\) 只能高 2 或 3，
  要"相差最多"就取高 2 的**最少**结点数 \\(N(2)=2\\) ⟹ 最多相差 \\(7-2=5\\)（另一半不能高 1，否则 \\(|BF(根)|=2\\)）；
- 反过来：n 个结点的 AVL 树高不超过 \\(\\lfloor 1.44\\log_2(n+1)\\rfloor\\) 量级，比普通 BST 的最坏 \\(n\\) 好得多。

## 考点提示
- 题目给一棵树问"插入某关键字后根是谁"（2021-6）：**先插到叶位置，再从下往上找最小不平衡子树**，注意先判型再旋；
- 题目给关键字序列问"哪条查找路径不可能"（2011-7）、"哪个序列不能生成此 BST"（2020-5）：抓住
  **查找路径上相邻两数的大小关系必须与转向一致**、以及 BST 的中序有序性；
- 数 BF 为 0 的分支结点（2013-3）：建完树后逐个子树算高度，别漏掉根。
`,
