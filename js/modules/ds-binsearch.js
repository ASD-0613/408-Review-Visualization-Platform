 ...
 * ----------------------------------------------------------------------------
 * 真题考情：18 年中 8 年考（2010-9 最大比较次数、2016-9 与顺序查找对比、
 * 2017-8 判定树形态、2023-8 600 元素最大比较次数、2024-5 适用的存储结构），
...
RC408.registerModule({
  ...
  theory: `
> **真题考情**：18 年中 8 年考（2010/2016/2017/2023/2024…），固定三问：
> **最大比较次数**（判定树高度）、**判定树形态**（与元素值无关，只与 n 有关）、
> **适用的存储结构**（有序**顺序表**，2024 年专考）。

## 折半查找的前提与过程
只适用于**有序**的**顺序存储**表（链表无法 O(1) 取中点）：
\`\`\`text
low = 0, high = n-1
while (low <= high):
    mid = ⌊(low + high) / 2⌋       # 教材取下整——决定判定树形态
    if a[mid] == target: 查找成功
    elif a[mid] < target: low = mid + 1    # 目标在右半区
    else: high = mid - 1                   # 目标在左半区
查找失败：low > high（走到判定树的外部结点）
\`\`\`

## 判定树（本模块的可视化主体）
- n 个元素 → **n 个内部结点（圆形）+ n+1 个外部结点（方形，查找失败到达）**；
- 树高（最大比较次数）= \(\lceil \log_2(n+1) \rceil\)：n=16 时为 5（2010 真题），n=600 时为 10（2023 真题）；
- 成功时**最少比较 1 次**（第一个 mid 就命中）；失败的比较次数 = 到达外部结点走过的层数；
- **形态唯一**：只要 n 相同，无论元素是什么值，判定树都一样（mid 只由下标决定）。

## 考点提示
- 元素**不在**表中时，比较次数同样 ≤ 树高，且失败位置唯一——可用来"反推序列"；
- 对比：顺序查找成功最多 n 次；折半 O(log₂n) 但要求有序 + 顺序存储（插入删除代价大）；
- 2016 年真题：数组末段附近的元素用"跨步递增 + 折半"的组合可能更省——灵活理解 mid 的移动。
`,
