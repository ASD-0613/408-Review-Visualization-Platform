...
RC408.registerModule({
  ...
  theory: `
> **为什么要有它**：多个进程共享临界资源时，必须保证"同一时刻只有一个进程进临界区"（**互斥**），
> 且彼此要按正确次序推进（**同步**）——信号量就是这两个需求的统一实现。
> **怎么实现**：给每类资源设一个记录型信号量（整型值 + 等待队列），只提供 P/V 两个**原子**操作。
> **记住什么**：三个信号量的分工（mutex 管互斥、empty/full 管同步）+ **P 的顺序不能颠倒**。

## 信号量机制（记录型信号量）
- **P(S) / wait(S)**：S = S − 1；若 **S < 0**，调用者**阻塞**并进 S 的等待队列（原子）；
- **V(S) / signal(S)**：S = S + 1；若 **S ≤ 0**，从等待队列唤醒一个进程（原子）；
- 值的含义：**S > 0** = 还剩 S 个可用资源；**S < 0** 时 **|S| = 正在等待的进程数**。
...
## 考点提醒（易错点）
1. **P(empty) 与 P(mutex) 的顺序不能颠倒**：先 P(mutex) 再 P(empty)，缓冲区满时生产者握着
   mutex 等空位、消费者又进不了临界区取产品 → **死锁**（V 的顺序交换无所谓，只影响效率）。
2. 初值的物理意义必须写出来：mutex=1（临界区只有 1 个）、empty=N（初始 N 个空位）、full=0（初始无产品）。
3. 经典变形：多生产者-多消费者、消费者须**连续取 k 件**（2014-47）、**信箱问题**（2015-45）
   ——核心永远是"资源计数 + 互斥锁"两个角色的搭配。
4. 答题模板（固定三问）：列互斥资源 → 给每个信号量写初值与含义 → 写 P/V 伪代码。

> **真题考情**：**13/18 年大题**（2009-45、2011-45、2013-45、2014-47、2015-45、2017-46、2019-43、
> 2020-45、2021-45、2022-46、2023-45、2024-46、2025-45），每题 7~8 分——OS 必考第一题。
`,
