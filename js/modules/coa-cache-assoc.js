 ...
 * ----------------------------------------------------------------------------
 * 真题考情：Cache 大题几乎每年一道（2010/2011/2016/2018/2020/2023/2025…），
 * 其中**组相联**是近年主流（2020-44 8路、2022-16 比较器、2023-43 4路、2025-43 8路），
...
RC408.registerModule({
  ...
  theory: `
> **真题考情**：Cache 18 年中 13 年考、**大题几乎每年**；直接映射（见另一模块）之后，
> **组相联是近年大题的主流形态**（2020-44 8路、2023-43 4路、2025-43 8路），
> 2022-16 还考了"比较器个数与位数"。

## 组相联 = 直接映射与全相联的折中
\`\`\`text
组数 = Cache 总行数 ÷ 路数
组号 = 主存块号 mod 组数          ← "直接定组"
组内：任意一个空闲路可放；都满时按 LRU 淘汰一组内最久未用的行
\`\`\`
- 路数 = 1 → 退化为**直接映射**；路数 = 行数 → 退化为**全相联**（组号字段消失）；
- **比较器**：组内每一路需要一个 Tag 比较器，并行比较 → 路数 = 比较器个数（2022 真题）；
- LRU 实现需要每行附加"最近使用"计数/位；命中率随路数增大而提高，硬件成本也随之上升。

## 地址划分（与直接映射的差异）
主存地址 = **标记 Tag + 组号 Index + 块内偏移 Offset**：
- 组号位数 = log₂(组数)；Tag 位数 = 块号位数 − 组号位数（**比直接映射更短**，因为组内有了选择余地）；
- 全相联：Index 为 0 位，Tag = 完整块号。

## 考点提示
- 给地址序列推 LRU 替换过程与最终命中率（本模块单步可练）；
- "某主存块会映射到哪一组"——块号 mod 组数，与 Tag 无关；
- 总容量计算别忘了每行的**有效位 + LRU 位**（路数 > 1 时需要）。
`,
