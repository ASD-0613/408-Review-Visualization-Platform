...
RC408.registerModule({
  ...
  theory: `
> **为什么要有它**：磁盘按"块"读写，二叉树一个结点只带一个关键字太亏——**m 阶 B 树让一个结点装一批关键字、带 m 棵子树**，把树高压到 \\(\\log_m N\\)，磁盘访问次数随之下降。
> **怎么实现**：结点内关键字有序、子树夹在相邻关键字之间；插入**一定落到最底层叶结点**，结点关键字数达到 m 就**分裂**、中间关键字上升；删除若下溢则先**向兄弟借**，借不到再**合并**，只有根被摘空才降高。
> **记住什么**：关键字数 / 高度 / 结点数的**四条公式**（下表）+ 插入分裂、删除借位与合并 + B 树与 B+ 树的辨析。

## 定义（m 阶 B 树，408 口径）
- 每个结点**至多 m 棵子树、m−1 个关键字**；
- **非根**结点至少 \\(\\lceil m/2 \\rceil\\) 棵子树、至少 \\(\\lceil m/2 \\rceil-1\\) 个关键字；根至少 1 个关键字（至少 2 棵子树）；
- 结点内关键字有序，第 i 棵子树的关键字全部落在 \\(k_{i-1}\\) 与 \\(k_i\\) 之间；
- **所有叶结点（失败结点）在同一层**——B 树是"绝对平衡"的多路查找树。

## 必背公式（右侧统计卡实时给出，可自行核对）
| 量 | 公式 |
| --- | --- |
| 结点关键字数 | 非根 \\(\\lceil m/2 \\rceil-1\\sim m-1\\)；根 \\(1\\sim m-1\\) |
...
## 插入：一路下沉，溢出就分裂
1. 从根出发按关键字大小选分支，**一定插入到最底层的叶结点**；
2. 该结点关键字数达到 m（> m−1）即**分裂**：取 \\(\\lfloor m/2 \\rfloor\\) 位置的关键字**上升到父结点**，左右两半各自成结点；
3. 父结点可能因此再溢出 → **分裂向上传播**；若根溢出则产生新根，**树高 +1**（B 树长高的唯一途径）。

## 删除：先借后合，只有根空了才降高
- 若删除的关键字在**非叶结点**：用其**前驱（左子树最右下）或后继**顶替，再回到叶结点删掉那个前驱 / 后继；
- 叶结点关键字数 < \\(\\lceil m/2 \\rceil-1\\)（**下溢**）：
  - **借位**：兄弟"富余"（> \\(\\lceil m/2 \\rceil-1\\)）→ 父关键字下移、兄弟最大 / 最小关键字上移，**树高不变**；
  - **合并**：兄弟也不富余 → "左兄弟 + 父关键字 + 本结点"并成一个结点，父结点少一个关键字并可能继续下溢；
- 根结点被摘空 → 唯一孩子成为新根，**树高 −1**（B 树降高的唯一途径）。

## B 树 vs B+ 树（辨析必考）
| 对比项 | B 树 | B+ 树 |
| --- | --- | --- |
| 关键字分布 | 每个结点都存，**不重复** | 非叶结点只作索引，**关键字在叶结点重复出现** |
...
## 考点提醒（易错点）
1. "**插入一定发生在叶结点**""**删除非叶关键字要用前驱 / 后继顶替**"是直接考法；
2. B+ 树支持顺序查找、B 树不支持；磁盘读写代价低、更适合文件索引的是 B+ 树；
3. 算"最多 / 最少关键字数、结点数、树高"一律套上面四条公式，**别凭感觉画图**；
4. 同一组关键字，**插入顺序不同则树形不同**，但高度区间被公式锁死。

> **真题考情**：**12/18 年（全为选择题，18 年无大题）**：2009-8 2012-9 2013-10 2014-9 2016-10
> 2017-9 2018-8 2020-10 2021-9 2022-8 2023-7 2025-8；题眼集中在关键字与高度上下界、插入分裂、删除调整、B+ 树辨析。
`,
