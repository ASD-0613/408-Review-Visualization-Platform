 ...
 * 考情：17 年中 13 年考（09-43、12-43 大题算占比；11-22、13-22、16-44、19-21、22-21 选择）。
...
RC408.registerModule({
  ...
  theory: `
> **真题考情**：18 年中 **13 年**考 I/O（2009-43、2012-43 大题算占比；2010-22、2011-22、
> 2013-22、2016-44、2019-21、2022-21、2024-22、2025-21 选择）——"CPU 用于 I/O 的时间占比"
> 是固定大题小问。

## 三种方式的 CPU 开销
| 方式 | CPU 做什么 | 占比公式 |
| --- | --- | --- |
| **程序查询** | 一直执行查询循环直到设备就绪 | 占比 = 查询频率 × 每次查询周期数 ÷ CPU 频率 |
| **程序中断** | 每传输一次打断一次（保存现场+服务+恢复） | 占比 = 中断次数/秒 × 每次中断周期数 ÷ 频率 |
| **DMA** | 仅预处理 + 后处理（传输由 DMA 控制器完成，周期挪用） | 占比 = (预处理+后处理) × 次数 ÷ 频率 |

## 大题套路（2009/2012 真题）
1. 由设备速率换算"每秒传输次数"（如 0.5MB/s ÷ 4B/次 = 125000 次/s）；
2. 中断方式：次数 × 每次中断周期（含现场保存，题给如 18+2 条指令 × CPI）；
3. DMA：只算预处理/后处理（如 500 周期/块），**传输期间 CPU 照常执行程序**（周期挪用）；
4. 除以 CPU 每秒总周期（主频）得占比。

## 考点提示
- DMA 传输期间 CPU **仍可执行程序**（与 DMA 争用_cache/总线时才暂停，2012-43 问优先级）；
- **DMA 请求优先级高于 CPU 取指**（不丢失数据）；键盘/打印机不适合 DMA（2025-21）；
- 中断服务程序做数据搬运（每次几个字节），慢但省硬件。
`,
