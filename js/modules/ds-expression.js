...
RC408.registerModule({
  ...
  theory: `
> **为什么要有它**：中缀式带括号和优先级，机器直接算很麻烦——**转成后缀式后，运算符出现的次序就是计算次序，且不再需要括号**，一个栈就能求值。
> **怎么实现**：中缀转后缀用**运算符栈**（操作数直接输出、运算符弹掉栈顶优先级不低于它的、右括号弹到左括号）；后缀求值用**操作数栈**（遇运算符弹两个数，先弹出的是右操作数）。
> **记住什么**：转换规则表 + **"增减括号"速算法** + **后缀式中操作数的相对次序不变**。
...
| 运算符 | 弹出栈顶所有**优先级 ≥ 当前**的运算符并输出（乘除高于加减），再把当前运算符入栈 |
| 扫描结束 | 栈内剩余运算符依次弹出输出 |

## 后缀表达式求值（操作数栈）
从左向右扫描：操作数入栈；遇运算符弹出两个操作数——**先弹出的是右操作数**——计算 \\(a\\;op\\;b\\) 后把结果压回；扫描结束时栈中唯一元素即表达式的值。

## 必背结论
- 后缀式中运算符的次序即**实际计算顺序**，且**不含括号**；中缀转后缀前后**操作数的相对次序不变**；
- **增减括号速算法**：先按运算优先级给中缀式全部加上括号 → 把运算符移到对应括号的**外面（后缀）/ 前面（前缀）** → 去掉全部括号即得；
- 同级运算符**左结合**：先入栈的先弹出（\\(a-b-c\\) → \\(ab-c-\\)，不是 \\(abc--\\)）；
- "扫描到某符号时栈内元素依次是什么"、"运算符栈的最大深度"都只需按规则逐符号模拟。

## 考点提醒（易错点）
1. **左括号只入栈、弹出时不输出**；遇右括号要一直弹到左括号为止；
2. 求值时**先弹出的是右操作数**——减法、除法写反就全错；
3. 前缀式（波兰式）把运算符放在前面，规则对称；给定后缀式也能借**表达式树**反推中缀式；
4. 后缀式的**操作数顺序与原中缀式完全一致**，可用来快速排除选项。

> **真题考情**：**4/18 年（选 3 题 + 大 1 道）**：选 2012-2、2014-2（扫描到某符号时栈内的元素）、
> 2024-2（求等价的后缀式）；大 2017-41（由表达式树写出等价中缀式，必要时加括号）。
`,
