...
- 翻译（编译/汇编，一次性生成）与解释（逐条，边解释边执行）：高级语言→汇编→机器语言。` },
      ...
      },
      {
        ...
        topics: [
          ...
          { id: 'coa-baseconv', label: '进位计数制与转换', status: 'theory', note: `> **真题考情**：进位转换每年嵌入大题（机器数、地址计算），2024-13、2025-12 直接考。

## 要点
- 二↔八：3 位一组；二↔十六：4 位一组（小数点为界两侧分组补零）；
- 十进制整数 → R 进制：**除基取余、倒序**；小数：**乘基取整、正序**（十进制小数未必精确，如 0.3）；
- 按权展开求和。

**提示**：2^k 进制间转换是免计算的重组（2025 考十六进制与二进制位数关系）；十进制小数给定位数后截断存在误差（2024）。` },
...
**提示**：尾数溢出可右规挽救，**阶码上溢才是真溢出**。` },
          { id: 'coa-muldiv', label: '乘除运算电路', status: 'theory', note: `> **真题考情**：2020-43 大题（无乘法指令实现乘法）、2025-44（补码除法器 idiv 异常）。

## 乘法（移位+加法）
- 部分积右移加被乘数：n 位乘法 = n 次"加法+右移"；阵列乘法器组合逻辑一遍完成；
- 补码乘法用布斯算法（Booth），记思想即可。

## 除法
- 原码除法：恢复余数法 / **加减交替法**（不恢复，更高效）；
- 溢出：除数为 0 或商超范围 → 触发异常（2025 真题 idiv）。` },
      ...
      },
      {
        ...
        topics: [
          ...
          { id: 'coa-mainmem', label: '主存与 CPU 连接（芯片扩展）', status: 'theory', note: `> **真题考情**：2009-15、2010-15、2015-16、2021-15 考芯片扩展计算；2023-15 考地址范围。

## 扩展方式
- **位扩展**：数据位数不够 → 数据端并联、地址共用（2 片 8K×8 → 8K×16）；
- **字扩展**：单元数不够 → 高位地址译码选片（4 片 8K×8 → 32K×8）；字位同时扩展 = 先位成组再字扩展。

## 计算要点
- 芯片数 = 总容量 ÷ 单片容量；MAR 位数 = 总单元数取 log；MDR = 存储字长；
- 地址范围按题目分配 ROM/RAM 位置（2023-15：RAM 低位、ROM 高位求 ROM 范围）。` },
...
**公式链**：距离 ↔ 传播时延 ↔ 争用期 ↔ 最小帧长，知三求一（2010 大题）。` },
          { id: 'net-switch', label: '以太网交换机自学习', status: 'theory', note: `> **真题考情**：2009-36、2011-47、2013-38、2016-35、2021-47、2024-35。

## 交换机自学习
- 收帧：记 **源 MAC → 入端口**；查**目的 MAC**：命中且端口不同 → 转发；命中同端口 → 过滤；无表项 → **泛洪**（除入口外全部）；
- 表项老化；每端口独立冲突域；隔离冲突域但**不隔离广播域**；
- **VLAN** 按端口划分隔离广播域：跨 VLAN 单播/广播都不转发（2022/2024 真题）。

**辨析**：Hub 域内所有主机都能收到帧（2016-35）；交换机全双工无冲突，不需要 CSMA/CD。` },
        ],
      },
      {
        name: '第4章 网络层',
...
- rwnd = 0 → 停发 + **持续计时器**零窗口探测。` },
      ...
      },
      {
        name: '第6章 应用层',
        topics: [
          { id: 'net-dns', label: 'DNS 域名解析过程', status: 'theory', note: `> **真题考情**：2010-40（递归条数）、2016-40（迭代次数）、2020-40（最短/最长时间）。

- **递归**：主机 → 本地服务器，替它跑到底（主机只发 1 条）；
- **迭代**：本地依次问根 → 顶级 → 权限（2016-40：可能 0 ~ 3 次外查）；
- 最短 = 本地缓存命中；最长 = 全链迭代 + 各级往返（2020-40 组合 Web 时延）。

**要点**：域名 → IP；层次域名空间（根/顶级/权限）；UDP 53（区域传送 TCP）；本地缓存大幅减少查询（2010-40）。` },
          { id: 'net-http', label: 'HTTP 请求与持久连接', status: 'theory', note: `> **真题考情**：2010-40（Web 时延）、2015-40（Cookie）、2022-40（持久连接）、2024-40（7 图像 RTT）。

## RTT 模板
- **非持久（HTTP/1.0）**：每对象 2 RTT（握手 1 + 请求 1）；N 图像 → 2(N+1) RTT（2024-40：7 图 → 16 RTT）；
- **持久非流水线（1.1）**：建连 1 RTT + 每对象 1 RTT（2011：5 图 = 7 RTT）；
- **持久流水线**：建连 1 + 总请求 1 ≈ 3 RTT 起（2022-40 最少时间）。

**Cookie**：用户跟踪/购物车/个性化——**缩短响应时间不属于**用途（2026-40）。` },
          { id: 'net-mail', label: '电子邮件与 FTP（SMTP/POP3）', status: 'theory', note: `> **真题考情**：2009-40（FTP 双连接）、2012-40（SMTP 阶段）、2017-40（FTP 端口）、2025-40（POP3）。

## 电子邮件
- 组成：用户代理 + 邮件服务器 + 协议（**发 = SMTP 推，收 = POP3/IMAP 拉**）；
- SMTP：TCP 25，**只能传 7 位 ASCII**（二进制走 MIME，2018-40）；用户代理→服务器→服务器全程 SMTP（2012-40）；
- POP3：TCP 110，一条连接可收多封（2025-40）；IMAP 在服务器上管理邮件。

## FTP
**控制连接（21，全程）+ 数据连接（20/协商端口，按需）**带外传送控制（2009-40）；主动模式服务器才主动发起数据连接（2017-40）。` },
        ],
      },
    ],
  },
